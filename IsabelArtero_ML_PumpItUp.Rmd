...
Mismo modelo que hice con mis compañeros de grupo 
...
```{r submission ranger1}
pred_ranger1 <- predict(modelo_ranger1, mi_test)
ia_sub_ranger1 <- data.frame(
                     id = mi_test$id,
                     status_group = pred_ranger1$prediction
                     )
fwrite(ia_sub_ranger1, file= "ia_sub_ranger1BIS.csv", sep=",")
...
```{r submission ranger gs}
pred_ranger250 <- predict(modelo_ranger250, mi_test)
ia_sub_ranger250 <- data.frame(
                     id = mi_test$id,
                     status_group = pred_ranger250$prediction
                     )
fwrite(ia_sub_ranger250, file= "ia_sub_ranger250.csv", sep=",")
...
```{r prediction xgb1}
test_max <- xgb.DMatrix(data.matrix(mi_test))

prediccion_xgb <- predict(modelo_xgb, test_max, reshape = T)
colnames(prediccion_xgb) <- c("functional", "functional needs repair", "non functional")
  
prediccion_xgb$prediction = apply(prediccion_xgb, 1 , function(x) colnames(prediccion_xgb)[which.max(x)])

```

```{r submission xgb1}
# Submission 
ia_sub_xgb <- data.frame(
                     id = mi_test$id,
                     status_group = prediccion_xgb$prediction
                     )
fwrite(ia_sub_xgb, file= "ia_sub_xgb.csv", sep=",")
...
prediccion_xgb_2 <- predict(modelo_xgb_2, test_max, reshape = T)
colnames(prediccion_xgb_2) <- c("functional", "functional needs repair", "non functional")
  
prediccion_xgb_2$prediction = apply(prediccion_xgb_2, 1 , function(x) colnames(prediccion_xgb_2)[which.max(x)])

```

```{r submission xgb2}
# Submission 
ia_sub_xgb_2 <- data.frame(
                     id = mi_test$id,
                     status_group = prediccion_xgb_2$prediction
                     )
fwrite(ia_sub_xgb_2, file= "ia_sub_xgb2.csv", sep=",")
